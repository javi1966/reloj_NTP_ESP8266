...
void reqOpenweather() {
  ...
  http.begin("http://api.openweathermap.org/data/2.5/forecast?q=sevilla,ES&APPID=c2ecf0a83c555c2054704fd94ff29f9e&mode=json&units=metric&cnt=2");
  int httpCode = http.GET();
  //Check the returning code
  if (httpCode > 0) {

    Serial.println(http.getString());
    const size_t capacity = 2 * JSON_ARRAY_SIZE(1) + JSON_ARRAY_SIZE(2) + 4 * JSON_OBJECT_SIZE(1) + 3 * JSON_OBJECT_SIZE(2) + 2 * JSON_OBJECT_SIZE(4) + JSON_OBJECT_SIZE(5) + 2 * JSON_OBJECT_SIZE(7) + JSON_OBJECT_SIZE(8) + 2 * JSON_OBJECT_SIZE(9) + 590;
